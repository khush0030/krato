...
const INSIGHT_CONFIG: Record<InsightType, { color: string; bg: string; icon: any; label: string }> = {
  win:         { color: '#22c55e', bg: 'rgba(34,197,94,0.1)',  icon: TrendingUp,    label: 'Win' },
  warning:     { color: '#ef4444', bg: 'rgba(239,68,68,0.1)',  icon: AlertTriangle,  label: 'Warning' },
  opportunity: { color: '#f59e0b', bg: 'rgba(245,158,11,0.1)', icon: Lightbulb,      label: 'Opportunity' },
  tip:         { color: '#3b82f6', bg: 'rgba(59,130,246,0.1)', icon: Zap,            label: 'Tip' },
};
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('### ')) {
      ...
      while (i < lines.length && (lines[i].startsWith('- ') || lines[i].startsWith('* '))) {
        listItems.push(<li key={i} style={{ color: c.textSecondary, fontSize: 14, lineHeight: 1.6, marginBottom: 3 }}>{formatInline(lines[i].slice(2), c)}</li>);
        i++;
      }
      ...
      while (i < lines.length && /^\d+\.\s/.test(lines[i])) {
        listItems.push(<li key={i} style={{ color: c.textSecondary, fontSize: 14, lineHeight: 1.6, marginBottom: 3 }}>{formatInline(lines[i].replace(/^\d+\.\s/, ''), c)}</li>);
        i++;
      }
  ...
  return (
    <>
      {parts.map((part, i) => {
        ...
        if (part.startsWith('`') && part.endsWith('`')) return <code key={i} style={{ backgroundColor: c.bgTag, color: '#a78bfa', padding: '1px 5px', borderRadius: 4, fontSize: 12, fontFamily: 'monospace' }}>{part.slice(1, -1)}</code>;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
        {[1, 2, 3, 4].map(i => (
          <div key={i} style={{ backgroundColor: c.bgCard, borderRadius: 12, padding: 20, border: `1px solid ${c.border}` }}>
            <div style={{ height: 14, width: '40%', backgroundColor: c.bgTag, borderRadius: 6, marginBottom: 12, animation: 'pulse 1.5s ease-in-out infinite' }} />
            <div style={{ height: 18, width: '80%', backgroundColor: c.bgTag, borderRadius: 6, marginBottom: 10, animation: 'pulse 1.5s ease-in-out infinite' }} />
            <div style={{ height: 12, width: '100%', backgroundColor: c.bgTag, borderRadius: 6, marginBottom: 6, animation: 'pulse 1.5s ease-in-out infinite' }} />
            <div style={{ height: 12, width: '60%', backgroundColor: c.bgTag, borderRadius: 6, animation: 'pulse 1.5s ease-in-out infinite' }} />
          </div>
        ))}
      </div>
    );
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', textAlign: 'center' }}>
        ...
        <button
          ...
          style={{ padding: '12px 24px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #7c3aed, #4f46e5)', color: 'white', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}
        >
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <button
          onClick={generateInsights}
          disabled={generating}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 8, border: `1px solid ${c.border}`, backgroundColor: c.bgCard, color: generating ? c.textMuted : c.text, fontSize: 13, fontWeight: 500, cursor: generating ? 'not-allowed' : 'pointer', opacity: generating ? 0.7 : 1 }}
        >
      ...
      {generating && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40, marginBottom: 16 }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            <RefreshCw size={24} color="#7c3aed" className="animate-spin" />
              ...
              <div key={insight.id} style={{ backgroundColor: c.bgCard, borderRadius: 12, padding: 20, border: `1px solid ${c.border}`, borderLeft: `3px solid ${config.color}` }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 600, color: config.color, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{config.label}</span>
                  </div>
                  <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, backgroundColor: insight.priority === 'high' ? 'rgba(239,68,68,0.1)' : insight.priority === 'medium' ? 'rgba(245,158,11,0.1)' : c.bgTag, color: insight.priority === 'high' ? '#ef4444' : insight.priority === 'medium' ? '#f59e0b' : c.textMuted, fontWeight: 600, textTransform: 'uppercase' }}>
                    {insight.priority}
                  </span>
                </div>
                ...
                {(insight.metric || insight.change) && (
                  <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                    {insight.metric && (
                      <span style={{ fontSize: 12, padding: '3px 10px', borderRadius: 6, backgroundColor: c.bgTag, color: c.text, fontWeight: 600 }}>
                        {insight.metric}
                      </span>
                    )}
                    {insight.change && (
                      <span style={{ fontSize: 12, padding: '3px 10px', borderRadius: 6, backgroundColor: insight.change.startsWith('+') ? 'rgba(34,197,94,0.1)' : insight.change.startsWith('-') ? 'rgba(239,68,68,0.1)' : c.bgTag, color: insight.change.startsWith('+') ? '#22c55e' : insight.change.startsWith('-') ? '#ef4444' : c.textMuted, fontWeight: 600 }}>
                        {insight.change}
                      </span>
                    )}
                  </div>
                ...
                {/* Action button */}
                {insight.action && (
                  <button style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#7c3aed', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: 0 }}>
        ...
        {(['insights', 'chat'] as const).map(tab => (
          <button
            ...
            style={{
              ...
              color: activeTab === tab ? '#7c3aed' : c.textMuted,
              background: 'none',
              border: 'none',
